...
## Read-in sheets
fn <- "data/20_0615-Grafica y Tablas Pruebas Moleculares COVID- semana 4 julio.xlsx"
...
tabs <- lapply(seq_along(sheets), function(i){
  ...
  tab <- tab %>% 
    ...
    mutate(laboratory = ifelse(laboratory == "Core Plus", "CorePlus", laboratory)) %>%
...
pr_by_lab <- dat %>%
  ...
  ungroup() %>%
  filter(n>0 & laboratory != "HRP") %>%
  select(-n)

save(pr_by_lab, file = "rdas/pr_by_lab-2020-07-06.rda")

pr <- pr_by_lab %>% 
  filter(laboratory %in% c("Toledo", "Ponce")) %>%
  group_by(date) %>%
...
save(pr, file = "rdas/pr-2020-07-06.rda")
