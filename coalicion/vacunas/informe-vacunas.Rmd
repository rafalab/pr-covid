...
fn <- "data/Vacunas (Raw Data - Clean) 06-06-2021.csv"
library(tidyverse)
library(readxl)
library(lubridate)
library(tidycensus)
...
\newpage
## Piramide poblacional de algunos pueblos

Vemos mucha variedad en la vacunación por municipio. Como ejemplo mostramos  la piramide poblacional para 
nuevo municipios.
...
tmp <- left_join(pop_by_municipio_age_gender, pop_by_municipio, by = "municipio") %>% 
  mutate(subpoblacion = poblacion*prop)
...
labs <- seq(-.08, .08, 0.04)
tab %>% filter(municipio %in% c("Aibonito", "San Juan", "Ponce", "Guaynabo", "Cataño", "Culebra", "Villalba",  "Loíza", "Mayagüez")) %>%
  ggplot(aes(ageRange, total, fill = estatus)) +
  geom_bar(position = "stack", stat = "identity", color = I("black"), width = 1) +
...
Parar una vacunación aparecer en este análisis se tiene que reportar al PREIS. Algunos proveedores toman diás en entrar los datos. La entrada de datos parece estar mejorando pero aun vemos rezago. La próxima gráfica muestra el por ciento de vacunaciones que se reportan como función del número de días. 
...
No incluye datos de las últimas dos semanas por estar incompletos. Noten que hay un rezago en reportar datos de vacunación al departamento de salud por lo caul mientras más reciente el día más  
...
Departamento de Salud por proveer datos.

Wilmarí De Jesus proveyó datos demográficos por pueblo.
