...
fn <- "data/Vacunas (Raw Data - Clean) 06-02-2021.csv"
library(tidyverse)
library(readxl)
library(lubridate)
library(tidycensus)
...
```{r}
tmp1 <- dose_1 %>% filter(date == max(dose_1$date)) %>% select(ageRange, gender, total) %>% rename(primera=total)
tmp2 <- dose_2 %>% filter(date == max(dose_1$date))%>% select(ageRange, gender, total) %>% rename(completa=total)
...
#ggsave("~/Desktop/formid.png", width = 8, height = 4)
```
...
ggsave("~/Desktop/formid.png", width = 10, height = 5)
